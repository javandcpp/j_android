 ...
 *
 */
@SuppressLint("NewApi") public class GestureDrawline extends View {
	private int mov_x;// 声明起点坐标
	private int mov_y;
	private Paint paint;// 声明画笔
	private Canvas canvas;// 画布
	private Bitmap bitmap;// 位图
	private ArrayList<GesturePoint> listPoints;// 装有各个view坐标的集合
	private ArrayList<Pair<GesturePoint, GesturePoint>> lineList;// 记录画过的线
	private HashMap<String, GesturePoint> autoCheckPointMap;// 自动选中的情况点
	private boolean isDrawEnable = true; // 是否允许绘制
	private ArrayList<GesturePoint> selectPoints; // 储存已选择的点，只选择一个点时使用
	...
	public GestureDrawline(Context context, ArrayList<GesturePoint> listPoints, boolean isVerify,
		...
		selectPoints = new ArrayList<GesturePoint>();
	}
	...
	private GesturePoint getGesturePointByNum(int num) {
		for (GesturePoint point : listPoints) {
			if (point.getNum() == num) {
				return point;
			}
		}
	...
	public void clearViewAnimation(){
		for (int i = 0; i < listPoints.size(); i++) {
			listPoints.get(i).image.clearAnimation();
//			listPoints.get(i).setPointState(Constants.POINT_STATE_NORMAL);
		}
	}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP:// 当手指抬起的时候
			if (isVerify) {
				...
				String code = AppUtil.md5Code(passWordSb.toString());
				if (passWord.equals(code)) {
					// 代表用户绘制的密码手势与传入的密码相同
					callBack.checkedSuccess();
				} else { // 用户绘制的密码与传入的密码不同。
					selectPoints.add(currentPoint); // 校验时添加已选择的当前的点
	...
	private void drawErrorPathTip() {
		...
		if(lineList.size() > 0) {
			...
			if(selectPoints != null) {
				selectPoints.clear(); // 清除选择的多个点
			}
		} else {
			if(selectPoints != null && selectPoints.size() == 1) {
				selectPoints.get(0).setPointState(Constants.POINT_STATE_WRONG);
				selectPoints.clear(); // 清除选择的一个点
			}
		}
